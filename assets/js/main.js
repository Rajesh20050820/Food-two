// Menu Data
const menuItems = [
  { name: 'Onion Capsicum Pizza', price: 99, category: 'Pizza', image: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&w=300&h=220&q=90', rating: 4.5, featured: true },
  { name: 'Cheese Paneer Pizza', price: 119, category: 'Pizza', image: 'https://www.dominos.co.in/files/items/Paneer_Special.jpg?auto=format&fit=crop&w=300&h=220&q=90', rating: 4.8, featured: true },
  ...
  { name: 'Cheese Burger', price: 59, category: 'Burger', image: 'https://www.mcdonaldsindia.com/images/products/product-cheese-burger.png?auto=format&fit=crop&w=300&h=220&q=90', rating: 4.4, featured: true },
  ...
  { name: 'Tiffin Meal (Dal, Chawal, Sabji, 5 Chapati, Achar)', price: 80, category: 'Tiffin', image: 'https://images.unsplash.com/photo-1606491956689-2ea866880c84?auto=format&fit=crop&w=300&h=220&q=90', rating: 4.7, featured: true },
...
document.addEventListener('alpine:init', () => {
  Alpine.data('menu', () => ({
    ...
    searchItems() {
      ...
      if (query) {
        this.filter = 'All';
        document.querySelectorAll('.filter-btn').forEach(btn => btn.classList.remove('bg-gold', 'text-navy'));
        document.querySelector('.filter-btn[data-category="All"]').classList.add('bg-gold', 'text-navy');
      }
    },
          ...
          if (cart[name] && cart[name].qty > 0) {
            controls.innerHTML = `
              <div class="flex items-center gap-2">
                <button class="p-2 bg-gold text-navy rounded-full hover:bg-white transition" onclick="Alpine.store('cart').removeItem('${name}')" aria-label="Decrease ${name} quantity">
                  <i class="fas fa-minus"></i>
                </button>
                <span class="text-lg font-semibold">${cart[name].qty}</span>
                <button class="p-2 bg-gold text-navy rounded-full hover:bg-white transition" onclick="Alpine.store('cart').addItem('${name}', ${item.price}, '${item.image}', '${item.category}')" aria-label="Increase ${name} quantity">
            ...
            controls.innerHTML = `
              <button class="px-4 py-2 bg-gold text-navy rounded-lg hover:bg-white transition" onclick="Alpine.store('cart').addItem('${name}', ${item.price}, '${item.image}', '${item.category}')" aria-label="Add ${name} to cart">
  ...
  Alpine.store('cart', {
    ...
    updateCart() {
      ...
      cartList.innerHTML = Object.keys(this.cart).length === 0 ? '<p class="text-center text-gray-400">Your cart is empty</p>' : '';
      let subtotal = 0;
      for (let name in this.cart) {
        const item = this.cart[name];
        const div = document.createElement('div');
        div.className = 'cart-item flex items-center p-4 rounded-lg shadow-md';
        div.innerHTML = `
          ...
          <div class="flex-1">
            ...
            <span class="block text-gold font-semibold">₹${(item.qty * item.price).toFixed(2)}</span>
          </div>
          <div class="flex items-center gap-2">
            <button class="p-2 bg-gold text-navy rounded-full hover:bg-white transition" onclick="Alpine.store('cart').removeItem('${name}')" aria-label="Decrease ${name} quantity">
              <i class="fas fa-minus"></i>
      ...
      billSummary.innerHTML = `
        <div class="flex justify-between mb-4 text-gray-200"><span>Subtotal</span><span>₹${subtotal.toFixed(2)}</span></div>
        <div class="flex justify-between mb-4 text-gray-200"><span>Tax (5%)</span><span>₹${tax.toFixed(2)}</span></div>
        <div class="flex justify-between mb-4 text-gray-200"><span>Delivery Charge</span><span>₹${deliveryCharge.toFixed(2)}</span></div>
        <div class="flex justify-between font-bold text-lg border-t pt-4 text-gold"><span>Total</span><span>₹${total.toFixed(2)}</span></div>
      `;
...
// Render Featured Carousel (Only for index.html)
if (window.location.pathname.includes('index.html') || window.location.pathname === '/') {
  const carousel = document.getElementById('featured-carousel');
  const featuredItems = menuItems.filter(item => item.featured);
  carousel.innerHTML = featuredItems.map(item => `
    <div class="featured-item bg-navy/50 backdrop-blur-md rounded-2xl shadow-lg p-4">
      <img src="${item.image}" alt="${item.name}" class="w-full h-48 object-cover rounded-lg mb-4" loading="lazy">
      <h3 class="text-lg font-playfair font-semibold text-white">${item.name}</h3>
      <div class="text-gold font-semibold text-lg">₹${item.price.toFixed(2)}</div>
      <div class="rating text-sm mt-1">
        ${[...Array(5)].map((_, i) => `<i class="fas fa-star ${i < Math.floor(item.rating) ? 'text-gold' : 'text-gray-400'}"></i>`).join('')} (${item.rating})
      </div>
      <button class="mt-4 px-4 py-2 bg-gold text-navy rounded-lg hover:bg-white transition" onclick="Alpine.store('cart').addItem('${item.name}', ${item.price}, '${item.image}', '${item.category}')" aria-label="Add ${item.name} to cart">
        <i class="fas fa-plus"></i> Add
      </button>
    </div>
  `).join('');
...
if (window.location.pathname.includes('index.html') || window.location.pathname === '/') {
  ...
  filterButtons.innerHTML = categories.map(category => `
    <button class="filter-btn px-4 py-2 rounded-lg ${category === 'All' ? 'bg-gold text-navy' : 'bg-white/10 text-white'} hover:bg-gold hover:text-navy transition" data-category="${category}" @click="filterMenu('${category}')">${category}</button>
  `).join('');
  menuDiv.innerHTML = categories.map(category => `
    <div class="category" x-ref="${category}" :class="{ 'collapsed': filter !== 'All' && filter !== '${category}' }">
      <h2 class="category-header text-3xl font-playfair font-bold text-gold cursor-pointer" @click="toggleCategory('${category}')" role="button" tabindex="0" aria-expanded="true">${category}</h2>
      <div id="items-${category}" class="menu-items grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ${menuItems.filter(item => category === 'All' || item.category === category).map(item => `
          <div class="menu-item rounded-2xl shadow-lg overflow-hidden" data-name="${item.name.toLowerCase()}">
            <img src="${item.image}" alt="${item.name}" class="w-full h-56 object-cover" loading="lazy">
            <div class="p-4">
              <h3 class="text-xl font-playfair font-semibold text-white">${item.name}</h3>
              <div class="text-gold font-semibold text-lg">₹${item.price.toFixed(2)}</div>
              <div class="rating text-sm mt-1">
                ${[...Array(5)].map((_, i) => `<i class="fas fa-star ${i < Math.floor(item.rating) ? 'text-gold' : 'text-gray-400'}"></i>`).join('')} (${item.rating})
              </div>
              <div id="total-${item.name.replace(/\s+/g, '-')}" class="text-sm text-gray-400 mt-2" aria-live="polite"></div>
            </div>
            <div id="controls-${item.name.replace(/\s+/g, '-')}" class="p-4 flex justify-between bg-white/10"></div>
          </div>
        `).join('')}
      </div>
    </div>
